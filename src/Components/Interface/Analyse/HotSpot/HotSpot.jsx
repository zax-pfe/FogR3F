...
const HotSpot = ({ data, coo, refBox }) => {
  ...
  const removeSelectedItem = useGameStore((state) => state.removeSelectedItem);
  ...
  const iAmCurrentHotspot = () => {
    return hotspotCurrent === data;
  };
  ...
  return (
    <>
      ...
      {selected && (
        <div
          ref={ref_pointMove}
          className={`${s.pointMove}`}
          style={{ left: coo.x, top: coo.y }}
        ></div>
      )}
    </>
  );
